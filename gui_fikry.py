...
with st.sidebar:
    ...
    # --- DETEKSI DATA BERUBAH ---
    if uploaded_file is not None:
        # Cek apakah file baru
    ...
    and st.session_state.selected_cabai in st.session_state.data.columns
    ):
        cabai_valid = True
    ...
    and st.session_state.selected_cabai in st.session_state.data.columns
        ...
        na_exists = st.session_state.data[
        st.session_state.selected_cabai
        ].isna().any()
    else:
...
st.progress(
    (current + 1) / len(tabs),
    text=f"({current + 1}/{len(tabs)})"
)

# ======================== TAB 1 ========================
#with tab1:
if current == 0:
    st.markdown("### 💾 Tampilan Impor Data")
...
with col_next:
    if st.button("Next ➡️", disabled=current == len(tabs) - 1):
        st.session_state.active_tab += 1
        st.rerun()
